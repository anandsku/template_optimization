...
while (1)
    ...
    elseif (length(findstr(lower(fl),'feedback information'))>0)
        ...
        while (1)
            ...
            elseif length(format)==2 % this takes care of the nre kind of rec files which write frequency at trig
                tpos = findstr(fl,'E+');
                tmpvec = [tmpvec;str2num(fl(1:tpos(1)+2))];
                tmpvec3 = [tmpvec3;str2num(fl(tpos(1)+3:tpos(2)+2))];
                tpos = findstr(fl,':');
                tmpvec2=[tmpvec2;(length(findstr(lower(fl(tpos+1:end)),'catch'))>0)];         
                
            else
                error(['error in reading rec file' fname])
            end
        end
        recdata.ttimes = tmpvec;
        recdata.catch  = tmpvec2;
        recdata.freq_at_trig=tmpvec3./10^3; 
